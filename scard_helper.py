...
class scard_class:
	...
	def parse_scard(self, scard_text):
		...
		output_type_id = getattr(self, "output_type")
...
def SCard_Entry(UserSubmissionID, timestamp, scard_dict):
	...
	for key in scard_dict:
		strn = "UPDATE Scards SET {0} = '{1}' WHERE UserSubmissionID = {2};".format(key,
		                                                                            scard_dict[key],
		                                                                            UserSubmissionID)
		utils.db_write(strn)
	utils.printer("SCard record added to database corresponding to UserSubmissionID {0}".format(
		UserSubmissionID))
